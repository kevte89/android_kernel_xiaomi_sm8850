...
static struct page *_page_read(struct address_space *mapping, pgoff_t index)
{
	return read_mapping_page(mapping, index, NULL);
}

int __partition_bm_read(struct file *dev, loff_t from,
		size_t len, void *buf)
{
	struct page *page;
	...
	while (len) {
		...
		page = _page_read(dev->f_mapping, index);
		if (IS_ERR(page))
			return PTR_ERR(page);
...
static int __partition_write(struct file *dev, const void *buf,
...
int _partition_bm_write(loff_t to, size_t len, const void *buf)
{
	int err;
	...
	err = __partition_write(bdev_file, buf, to, len);
...
static inline void _kill_final_newline(char *str)
{
...
int get_bootmonitor_devices(void)
{
	int err;
	int i;
...
int write_blackbox_header(struct bootmonitor_data_context *cxt, char *buffer, int event) {
	int err;
	int event_count;
	...
	err = __partition_bm_read(bdev_file, BLACK_BOX_OFFSET, sizeof(struct log_table_header), &black_box_log_table_header);
	if (err != 0) {
		MTN_PRINT_ERR("%s-%d:__partition_bm_read fail\n", __func__, __LINE__);
		return err;
	}
	...
	//printlogtableheader();
	err = _partition_bm_write(BLACK_BOX_OFFSET, sizeof(struct log_table_header), &black_box_log_table_header);
	if (err != 0) {
		MTN_PRINT_ERR("%s-%d:_partition_bm_write fail\n", __func__, __LINE__);
		return err;
	}

	/*update error_type_header*/
	err = __partition_bm_read(bdev_file, black_box_log_table_header.boot_content_offset, sizeof(struct log_error_type_header), &error_type_header);
	if (err != 0) {
		MTN_PRINT_ERR("%s-%d:__partition_bm_read fail\n", __func__, __LINE__);
		return err;
	}
	...
	/*update error_header*/
	err = __partition_bm_read(bdev_file, black_box_log_table_header.boot_content_offset + error_type_header.header_size + INIT_INDEX * HEADER_SIZE_4K,
	                        sizeof(struct log_error_header), &error_header);
	if (err != 0) {
		MTN_PRINT_ERR("%s-%d:__partition_bm_read faild\n", __func__, __LINE__);
		return err;
	}
	...
	//printerrorheader();
	err = _partition_bm_write(black_box_log_table_header.boot_content_offset + error_type_header.header_size + (INIT_INDEX + event_count)* HEADER_SIZE_4K,
                            sizeof(struct log_error_header), &error_header);
	if (err != 0) {
		MTN_PRINT_ERR("%s-%d:_partition_bm_write fail\n", __func__, __LINE__);
		return err;
	...
	err = __partition_bm_read(bdev_file, black_box_log_table_header.boot_content_offset + PARTITION_LOG_OFFSET,
	...
	err = _partition_bm_write(black_box_log_table_header.boot_content_offset, sizeof(struct log_error_type_header), &error_type_header);
	if (err != 0) {
		MTN_PRINT_ERR("%s-%d:_partition_bm_write fail\n", __func__, __LINE__);
		return err;
	}
	...
	//printbootheader();
	err = _partition_bm_write(black_box_log_table_header.boot_content_offset + PARTITION_LOG_OFFSET,
	                        sizeof(struct log_boot_monitor_header), &boot_monitor_header);
	if (err != 0) {
		MTN_PRINT_ERR("%s-%d:_partition_bm_write fail\n", __func__, __LINE__);
		return err;
...
static void add_timeout_header(char *oops_buf) {
    ...
    ret_len = snprintf(str_buf, BOOT_MONITOR_HEADER_SIZE,
            ...
            boot_index, bm_boot_events[bm_event_counts].name, bm_boot_mode, build_fingerprint, before_fingerprint, version_change, \
...
void bootmonitor_get_kmsg(char *buffer) {
...
/*vmap pstore*/
static void *bootmonitor_ram_vmap(phys_addr_t start, size_t size)
{
	struct page **pages;
...
void bootmonitor_get_pmsg(struct bootmonitor_data_context *cxt) {
    char *pmsg_buffer_start = NULL;
	struct bootmonitor_pmsg_buffer_hdr *p_hdr = NULL;
    phys_addr_t pstart;
	size_t dstart;
	...
	MTN_PRINT_INFO("pstart=0x%llx, dstart=0x%lx\n", pstart, dstart);
	MTN_PRINT_INFO("pmsg_buffer_start=0x%p\n", pmsg_buffer_start);
	if (!pstart || !dstart) {
		MTN_PRINT_ERR("pmsg_buffer_start is NULL\n");
		return;
	}

	pmsg_buffer_start = bootmonitor_ram_vmap(pstart, dstart);
	if (!pmsg_buffer_start) {
		MTN_PRINT_ERR("pmsg_buffer_start is NULL\n");
		return;
	}

	p_hdr = (struct bootmonitor_pmsg_buffer_hdr *)pmsg_buffer_start;
...
static int bootmonitor_parse_dt_u32(struct platform_device *pdev,
				const char *propname,
...
#define parse_u32(name, field, default_value) {				\
		ret = bootmonitor_parse_dt_u32(pdev, name, default_value,	\
					    &value);			\
		if (ret < 0)						\
			return ret;					\
		field = value;						\
