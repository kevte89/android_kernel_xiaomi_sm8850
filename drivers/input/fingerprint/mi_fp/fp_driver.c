...
static irqreturn_t qbt_ipc_irq_handler(int irq, void *dev_id)
{
	...
	if (fp_dev->intr1_trigger_high){
		if (gpio_get_value(fp_dev->fw_ipc.gpio)) {
			irq_set_irq_type(fp_dev->fw_ipc.irq, IRQ_TYPE_LEVEL_LOW);
			fp_local_time_printk("IPC IRQ_TYPE_LEVEL_LOW \n");
		} else {
			irq_set_irq_type(fp_dev->fw_ipc.irq, IRQ_TYPE_LEVEL_HIGH);
			fp_local_time_printk("IPC IRQ_TYPE_LEVEL_HIGH \n");
			return IRQ_HANDLED;
		}
	}
	...
	gpio_direction_input(fp_dev->fw_ipc.gpio);
  	...
  	if (fp_dev->intr1_trigger_high){
          rc = request_threaded_irq(fp_dev->fw_ipc.irq, NULL, qbt_ipc_irq_handler,
                          IRQF_ONESHOT |
                          IRQF_TRIGGER_HIGH, desc,
                          fp_dev);
          pr_err("IPC trigger is high\n");
        } else {
          rc = request_threaded_irq(fp_dev->fw_ipc.irq, NULL, qbt_ipc_irq_handler,
                          IRQF_ONESHOT |
                          IRQF_TRIGGER_FALLING, desc,
                          fp_dev);
          pr_err("IPC trigger is falling\n");
        }
				   ...
				   const char *buf, size_t count)
{
	...
	if (fp_dev->intr2_side_config == 1 && fp_dev->intr2_AP_pin_enable) {
		...
		if (!strncmp(buf, "0", strlen("0"))) {
			...
			pinctrl_select_state(fp_dev->pinctrl, fp_dev->pins_intr2_high);
			fp_dev->intr2_state = 1;
		} else {
...
static long fp_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
	...
	switch (cmd) {
		...
		case FP_IOC_RESET_PULL_FLAG:
			pr_debug( "FP_IOC_RESET_PULL_FLAG ======\n" );
			if(copy_to_user((u_int32_t __user *)arg, (u_int32_t *)fp_dev->rst_pull_flag, 5*sizeof(u_int32_t))) {
				pr_debug( "FP_IOC_RESET_PULL_FLAG failed!\n" );
				retval = -EFAULT;
				break;
			}
			break;
		...
		case FP_IOC_INTR2_ELECTRICAL_CONFIG:
			pr_debug( "FP_IOC_INTR2_ELECTRICAL_CONFIG ======\n" );
			if(copy_to_user((u_int32_t __user *)arg, (u_int32_t *)fp_dev->intr2_electrical_config, 5*sizeof(u_int32_t))) {
				pr_debug( "FP_IOC_INTR2_ELECTRICAL_CONFIG failed!\n" );
				retval = -EFAULT;
				break;
			}
			break;
		...
		case FP_IOC_REQUEST_RESOURCE:
			...
			if (!fp_dev->irq_request) {
				...
				int ret = request_threaded_irq(fp_dev->irq_num, NULL, fp_irq,
							...
							fp_dev);
				pr_err("INTR1 %d trigger is rising\n", fp_dev->irq_num);
		...
		case FP_IOC_DISABLE_INTR2:
			...
			if (fp_dev->intr2_side_config == 1 && fp_dev->intr2_AP_pin_enable) {
				...
				retval = -EINVAL;
				pr_err("intr2 is not init or config to AP\n");
			}
			break;
...
#endif
{
	...
	fp_dev->intr1_trigger_high = false;
	/* config fp power. */
...
#else
static void fp_remove(struct platform_device *driver_device)
#endif
{
	struct fp_device *fp_dev = &fp;
	FUNC_ENTRY();
