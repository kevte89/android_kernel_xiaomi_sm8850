...
#define FP_IOC_RESET_PULL_FLAG          _IOR(FP_IOC_MAGIC, 45, uint32_t)
#define FP_IOC_INTR2_SIDE_CONFIG        _IOR(FP_IOC_MAGIC, 46, uint32_t)
#define FP_IOC_INTR2_ELECTRICAL_CONFIG  _IOR(FP_IOC_MAGIC, 47, uint32_t)
...
struct fp_device {
	...
	u_int32_t rst_pull_flag[5];  // 0:disable  1:enble
	bool reset_pin_enable;
	bool intr2_AP_pin_enable;
	u_int8_t intr2_state;
	u_int32_t intr2_side_config; // 0:none  1:AP side  2:TIC side
	unsigned int intr2_electrical_config[5]; // 0:none 1:low electrical, 2:high electrical
	spinlock_t intr2_events_lock;
	bool intr1_trigger_high;
/**************************Pin******************************/
	signed irq_gpio;
	int irq_num;
	int irq_enabled;
	...
	struct pinctrl_state *pins_intr2_high;
	struct pinctrl_state *pins_intr2_low;
/**************************Pin******************************/
