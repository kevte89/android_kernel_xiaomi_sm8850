...
static int smc_setup_rd_ls_gpio(struct smc_core_data *cd)
{
	int rc;
	...
	if (gpio_is_valid(cd->gpios.rd_ls_gpio)) {
		rc = gpio_request(cd->gpios.rd_ls_gpio, "rsmc_soc_pwr");
		if (rc) {
			printk(KERN_ERR"%s: rd_ls_gpio gpio_request %d failed %d\n",
				__func__, cd->gpios.rd_ls_gpio, rc);
			return rc;
		}

		rc = gpio_get_value(cd->gpios.rd_ls_gpio);
		printk(KERN_INFO"%s: rd_ls_gpio %d value %d",
			__func__, cd->gpios.rd_ls_gpio, rc);

		rc = gpio_direction_output(cd->gpios.rd_ls_gpio, GPIO_LOW);
		if (rc) {
			printk(KERN_ERR"%s: rd_ls_gpio %d output high err %d",
				__func__, cd->gpios.rd_ls_gpio, rc);
			return rc;
		}
	...
	struct smc_core_data *cd)
{
	...
	value = of_get_named_gpio(smc_node, "rd_ls_gpio", 0);
	printk(KERN_INFO"rd_ls_gpio = %d", value);
	cd->gpios.rd_ls_gpio = value;
